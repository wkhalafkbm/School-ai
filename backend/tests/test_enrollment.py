...
# ---------------------------------------------------------------------------
# Issue #45 — GET /api/enrollment/profile/stream (SSE)
# ---------------------------------------------------------------------------
...
def test_enrollment_profile_stream_returns_event_stream_content_type(client, monkeypatch):
    monkeypatch.setenv("AI_MODE", "scripted")

    with client.stream("GET", "/api/enrollment/profile/stream") as response:
        assert response.status_code == 200
        assert response.headers["content-type"].startswith("text/event-stream")


def test_enrollment_profile_stream_scripted_mode_yields_base_field_done_without_contacting_orchestrate(
    client, monkeypatch
):
    monkeypatch.setenv("AI_MODE", "scripted")
    monkeypatch.setenv("WXO_BASE_URL", WXO_BASE)
    ...
    fallback_note = base_event[1]["suggested_schedule"]["note"]
...
def test_enrollment_profile_stream_live_mode_field_event_carries_live_note(client, monkeypatch):
    monkeypatch.setenv("AI_MODE", "live")
    monkeypatch.setenv("WXO_BASE_URL", WXO_BASE)
    monkeypatch.setenv("WXO_API_KEY", "test-key")
    monkeypatch.setenv("AGENT_ID_ENROLLMENT", AGENT_ENROLLMENT)
    ...
    assert base_event[1]["suggested_schedule"]["note"]
    assert field_event[1] == {
        "path": "suggested_schedule.note",
